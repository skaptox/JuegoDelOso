...
struct Move {
  ...
  Move() : row(0), col(0), state(STATE::EMPTY), dValue(0), hValue(0) {}
...
class Board {
  ...
  */
  int osoWordAround(int, int) const;
  int osoWordAround(const Move &mv) const;
  ...
  */
  std::vector<Move> squaresOsoAround(int r, int c) const;
